...
> **再帰と無限ループ**
>
> このように、再帰を取り扱う場合、`終了条件`を満たす可能性があるか、をちゃんと確認しないと、
> 容易に無限ループに陥る可能性がある。なので、再帰的なアプローチを取る場合は、最初に終了条件を
> 確認するところから実装する。この辺りの再帰実装のアプローチは、『プログラミングの基礎』
> (浅井 健一, サイエンス社, 2007) に詳しい。
...
　このように`let`を使う意味はいろいろある。まず一つに、このような整理を行うことで、実質的に見通しがよくなるということだ。同じような操作が現れるということは、同じような意味のデータが現れているともいえる。それらを整理して見通しを良くするために、`let`というのは都合がいい。

　また、グローバルに宣言するほどでもないが、あるスコープの中で一時的な関数を宣言したいという場合があるかもしれない。そういったときにも、`let`は便利だ。

```clojure
(defn let-in-function-example [x y]
  (let [two-mul (fn [target] (* target 2))]
    (+ (two-mul x) (two-mul y))))
(let-in-function-example 1 2) ;; => 6
```

　これは`(def foo (fn [] (do-anything)))`の形で、関数が定義できることを応用したものだ。このように、`let`はその内部だけで使われる関数を定義するのにも使える。

7. ループの洞窟
---------------

　とはいえ、洞窟の分岐を表現するための形式というのはこれだけではない。例えば連結リストというのも考えられるだろう。例えば
...
　といったような、一連の通路の繋がりを表現するような形式である。ここでは簡易的に、次のようなテストデータを用意しよう。
